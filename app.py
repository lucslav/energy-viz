...
with st.sidebar:
    ...
    st.header("⚙️ Energy Viz Tools")
    
    if not settings_df.empty:
        s = settings_df.iloc[0]
        st.subheader("Active Rates (Incl. VAT)")
        ...
        st.info("Setup rates to see details here.")

# --- MAIN CONTENT ---
if settings_df.empty:
    st.header("⚡ Energy Viz Setup")
    st.markdown("Enter your rates from your ESB/Provider bill (Net prices).")
    with st.form("setup_form"):
        ...
        with c2:
            ...
            st.write("VAT will be added automatically (9%).")
        if st.form_submit_button("Save Rates & Start"):
...
# --- HEADER ---
st.markdown(f'<div style="text-align:center; padding:20px; background:white; border-radius:15px; border:1px solid #eee; margin-bottom:20px;"><img src="{LOGO_URL}" width="60"><h2>Energy Viz</h2><p style="color:gray;">Interactive ESB Smart Meter Analytics</p></div>', unsafe_allow_html=True)
...
def render_tab(tab_idx, db_key, esb_name, color, is_kwh=False):
    with tabs[tab_idx]:
        df_sub = full_data[full_data['type'] == db_key].copy()
        
        with st.expander(f"📥 Browse & Upload: {esb_name}", expanded=df_sub.empty):
            up = st.file_uploader(f"Select {esb_name}", type="csv", key=f"up_{db_key}", label_visibility="collapsed")
            if up:
                raw = pd.read_csv(up)
        ...
        if not df_sub.empty:
            st.info(f"📂 **ESB Source:** {esb_name} | History: {df_sub['timestamp'].min().date()} to {df_sub['timestamp'].max().date()}")
            ...
            if is_kwh:
                ...
                c1.metric("Total Usage", f"{df_sub['value'].sum():.1f} kWh")
                c2.metric("Total Bill (Est.)", f"€{df_sub['Cost'].sum() + (days * s['standing_charge'] * v_mul):.2f}")
                c3.metric("Avg Daily Usage", f"{df_sub['value'].sum()/days:.2f} kWh")
                c4.metric("Avg Daily Cost", f"€{(df_sub['Cost'].sum()/days) + (s['standing_charge']*v_mul):.2f}")
                
                view = st.radio("Switch View:", ["kWh", "Euro (€)"], horizontal=True, key=f"v_{db_key}")
                y_col = 'value' if "kWh" in view else 'Cost'
            ...
            st.plotly_chart(fig, width="stretch") # Updated to width="stretch" as per 2026 logs

# Render ESB Tabs
render_tab(0, "CALC_KWH", "30-minute readings in calculated kWh", "#00CC96", is_kwh=True)
render_tab(1, "READ_KW", "30-minute readings in kW", "#FF4B4B")
render_tab(2, "DNP_SNAPSHOT", "Daily snapshot of day/night/peak usage in actual kWh*", "#636EFA")
render_tab(3, "TOTAL_SNAPSHOT", "Daily snapshot of total usage and export data in actual kWh", "#7f8c8d")
